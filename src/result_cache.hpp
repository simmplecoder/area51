...
namespace shino {
    ...
    public:
        ...
        template<typename ArgType, typename = std::enable_if_t<shino::is_straight_tuple<Key>::value>>
        ValueType& operator()(ArgType&& tuplearg) {
            if (tuplearg != current_key) {
                current_value = std::apply(current_callable, std::forward(tuplearg));
                current_key = tuplearg;
            }
